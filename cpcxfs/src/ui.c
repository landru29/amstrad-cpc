...
/* parse options */
	...
	optind = FIRST_OPTIND;
    while ((optchar=getopt(nbof_args+1,arg,"ftb"))!= -1) {
		switch (optchar) {
...
/* parse options */
	opterr = 0;	/* no errormessages in getopt */
	optind = FIRST_OPTIND;
    while ((optchar=getopt(nbof_args+1,arg,"awlu"))!= -1) {
		switch (optchar) {
...
/* parse options */
	opterr = 0;	/* no errormessages in getopt */
	optind = FIRST_OPTIND;
    while ((optchar=getopt(nbof_args+1,arg,"b:h:t:s:f:cdm12"))!= -1) {
		switch (optchar) {
...
/* parse options */
	...
	opterr = 0;	/* no errormessages in getopt */
	optind = FIRST_OPTIND;
    while ((optchar=getopt(nbof_args+1,arg,"f"))!= -1) {
		switch (optchar) {
		case 'f':	local_force = TRUE;	break;
		case '?':	return cmd_error(errmsg);
		}
...
/* KT changed so format names can be specified */
    while ((optchar=getopt(nbof_args+1,arg,"fsde"))!= -1) {
/*  while ((optchar=getopt(nbof_args+1,arg,"sdiv"))!= -1) { */
	    printf("optchr: %02x\n",optchar);
		switch (optchar) {
...
buf[0] = '\0';
	if (nbof_args==0) {
		...
		opterr = 0;	/* no errormessages in getopt */
		optind = FIRST_OPTIND;
        while ((optchar=getopt(nbof_args+1,arg,"fsd"))!= -1) {
			switch (optchar) {
			case 'd':
			{
...
/* parse options */
	...
	optind = FIRST_OPTIND;
	local_mode = mode;
    while ((optchar=getopt(nbof_args+1,arg,"f:ctb"))!= -1) {
		switch (optchar) {
